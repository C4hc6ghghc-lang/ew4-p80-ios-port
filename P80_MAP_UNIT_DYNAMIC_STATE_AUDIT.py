...
# P80 dynamic fire-cell presentation closure.
fire_native=N/'Effects'/'anim_fire_hd.png'
fire_frozen=L/'assets/effects'/'anim_fire_hd.png'
fire_asset_ok=same(fire_native, fire_frozen)
if not fire_asset_ok: errors.append('fire atlas frozen identity')
fire_core=(ROOT/'SOURCE_NATIVE/EW4_iOS_NativePort/NativeCore/Sources/EW4NativeCore/NativeBattleFirePresentationCore.swift').read_text()
fire_checks={
 'fire_core_exists': bool(fire_core),
 'six_frames': fire_core.count('.init(x:') >= 6 and 'frameMilliseconds = 160.0' in fire_core,
 'render_height_42': 'renderedHeight = 42.0' in fire_core,
 'alpha_0_86': 'alpha = 0.86' in fire_core,
 'atlas_route': 'anim_fire_hd.png' in scene,
 'fire_set_route': 'context.fireCells' in scene,
 'per_frame_update': 'updateNativeFirePresentation(nowMilliseconds: now)' in scene,
 'stale_remove': 'fireEffectNodes[key] = nil' in scene,
 'below_units': 'node.zPosition = 75' in scene,
}
if not all(fire_checks.values()): errors.append('native fire renderer contract')
report['battle_fire_overlay']={
 'asset_identical':fire_asset_ok,
 'native_sha256':sha(fire_native) if fire_native.is_file() else None,
 'frozen_sha256':sha(fire_frozen) if fire_frozen.is_file() else None,
 'contract':fire_checks,
}

report['errors']=errors
out=ROOT/'P80_MAP_UNIT_DYNAMIC_STATE_AUDIT.json'
...
print(json.dumps({
 ...
 'fire_overlay_ok':fire_asset_ok and all(fire_checks.values()),
},ensure_ascii=False))
